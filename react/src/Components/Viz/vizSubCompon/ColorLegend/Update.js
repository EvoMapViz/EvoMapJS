...
if(Colortype === 'continuous'){
    ...
    var unit = ""
    if(typeof colorSelMeta[0].unit !== 'undefined'){unit = colorSelMeta[0].unit}
    var height = 30 + yGap * Colorbins.length;
}

var width = 500;
if(winWidth < 768){width = 1000}
const svg = d3.select('.color-legend-svg-content-responsive')
...
svg.on('click', function(event){
    if(event.target.className.baseVal === 'color-legend-svg-content-responsive'){ // if click is not a legend item
...
function clickDiscrete(event){
    let tthis = svg
                .selectAll('.color_legend')
                .filter( function(e){ return d3.select(this).attr('data-name') === event })
...
function clickContinuous(event){
    ...
    let tthis = svg
                .selectAll('.color_legend')
                .filter( function(e){ return d3.select(this).attr('data-name') === event.toString()})
...
if(Colortype === 'discrete'){
    ...
    uniqueOptions.sort((a, b) => d3.ascending(a[colorSel], b[colorSel])).map(String)

    // !! scaleOrdinal for *discrete* (here), and scaleThreshold for continuous (below) => must be definied within each "if"
    const color = d3.scaleOrdinal() // https://stackoverflow.com/questions/20847161/how-can-i-generate-as-many-colors-as-i-want-using-d3
                    .domain(Colordomain)
                    .range(Colorrange) 
    ...
    .append("circle")
    .attr("cx", xDrift)
    .attr("cy", function(d,i){ return 15 + i*yGap})
    .attr("r", 7)
    ...
    .append("text")
    .attr("x", xDrift + 20)
    .attr("y", function(d,i){ return 15 + i*yGap})
    .text(function(d){ return truncate(d, 25)})
...
if(Colortype === 'continuous'){
    ...
    let xDrift = 180
     ...
     if(increasing === 'true'){
        ...
        .style("fill", function(d){return color(d)})
        .classed('color_legend', 'true')
        .attr('data-bounds', function(d,i){ return [extended_bins[i], extended_bins[i+1]] })
        .attr('data-name', d => d)
        .attr('data-clicked', 'false')
        .attr('cursor', 'pointer')
        .on('click', function(d, event){
        ...
        .attr("x", xDrift + 20)
        .attr("y", function(d,i){ return 15 + i*yGap})
        .text(function(d,i){ return extended_bins[i] + unit + " to " + extended_bins[i+1] + unit })
        .attr("text-anchor", "left")
        .style("alignment-baseline", "middle")
        .classed('color_legend', 'true')
        .attr('data-bounds', function(d,i){ return [extended_bins[i], extended_bins[i+1]] })
